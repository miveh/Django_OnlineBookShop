...
from cart.views import CartView, add_to_cart, remove_from_cart, quantity, quantity_next_cart, NextCartView, \
    ...
    return_all_to_cart, HistoryListView, anonymous_cart, add_to_session, anonymous_cart_remove, FactorsView, \
    OrdersView, report
...
urlpatterns = [
    ...
    path('staff/factors/', FactorsView.as_view(), name='factors_room'),
    path('staff/oeders/', OrdersView.as_view(), name='orders_room'),
    path('staff/report/', report, name='report')
]
